...
int main()
{
    cv::Mat camMatrix1 = (cv::Mat_<double>(3,3) <<
                          2.76540937e+03, 0.00000000e+00, 1.91385608e+03,
                          0.00000000e+00, 2.76009287e+03, 1.06885642e+03,
                          0.00000000e+00, 0.00000000e+00, 1.00000000e+00);
    cv::Mat camMatrix2 = (cv::Mat_<double>(3,3) <<
                          2.74600422e+03, 0.00000000e+00, 1.91950000e+03,
                          0.00000000e+00, 2.74600422e+03, 1.07950000e+03,
                          0.00000000e+00, 0.00000000e+00, 1.00000000e+00);
    cv::Mat distVect1 = (cv::Mat_<double>(1, 4) << -0.00172206, 0,0,0);
    cv::Mat distVect2 = (cv::Mat_<double>(1, 4) << 0.0156681, 0,0,0);
    cv::Mat rvec = (cv::Mat_<double>(3,3) <<
                    0.54242838, -0.42654515,  0.72376149,
                    0.42637967,  0.88208839,  0.20030092,
                    -0.72385899,  0.19994828,  0.66033995);
    cv::Mat tvec = (cv::Mat_<double>(3,1) <<
                    -12865.82851411,  -3226.09108778,   5397.60862407);
    cv::Mat points1 = (cv::Mat_<double>(2,1) << 2608., 1965.);
    cv::Mat points2 = (cv::Mat_<double>(2,1) << 1179., 2087.);
    cv::Mat points3d1, points3d2, err;

    triangulatePoints2(
      camMatrix1,  // 3x3 camera matrix of cam 1
      distVect1,   // 4x1 or 5x1 distortion coefficients
      camMatrix2,  // 3x3 camera matrix of cam 2
      distVect2,   // 4x1 or 5x1 distortion coefficients
      rvec,    // 3x3 Rotation matrix between the coordinate systems of the first and the second cameras.
      tvec,        // 3x1 or 1x3 Translation vector between coordinate systems of the cameras.
      points1,     // 2xN or 2-channel 1xN or Nx1 image points on photo taken by camera 1
      points2,     // 2xN or 2-channel 1xN or Nx1 image points on photo taken by camera 2
      points3d1,   // 1xN 3-channel triangulated based on cam-1 coordinate
      points3d2,   // 1xN 3-channel triangulated based on cam-2 coordinate
      err       );  // 1xN 1-channel, norm of error (between points and back-projected points) of each point

    cout << "Point 3d1:\n" << points3d1 << endl;
    cout << "Point 3d2:\n" << points3d2 << endl;

    cv::Mat r44 = cv::Mat::eye(4, 4, CV_64F);
    for(int i=0;i<3;i++)for(int j=0;j<3;j++)r44.at<double>(i,j)=rvec.at<double>(i,j);
    for(int i=0;i<3;i++) r44.at<double>(i, 3)=tvec.at<double>(i,0);
    cout << "r44:\n" << r44 << endl;
    cv::Mat r44inv = r44.inv();
    cout << "r44inv:\n" << r44inv << endl;
    cv::Mat Point4d1 = cv::Mat::ones(4, 1, CV_64F);
    Point4d1.at<double>(0, 0) = points3d1.at<cv::Point3d>(0).x;
    Point4d1.at<double>(1, 0) = points3d1.at<cv::Point3d>(0).y;
    Point4d1.at<double>(2, 0) = points3d1.at<cv::Point3d>(0).z;
    cout << "point 4d1:\n" << Point4d1 << endl;
    cv::Mat Point4d2 = cv::Mat::ones(4, 1, CV_64F);
    Point4d2.at<double>(0, 0) = points3d2.at<cv::Point3d>(0).x;
    Point4d2.at<double>(1, 0) = points3d2.at<cv::Point3d>(0).y;
    Point4d2.at<double>(2, 0) = points3d2.at<cv::Point3d>(0).z;
    cout << "point 4d2:\n" << Point4d2 << endl;
    cout << "Transformed: \n" << r44inv * Point4d2 << endl;

    cv::Mat r4_c1 = (cv::Mat_<double>(4,4) <<
            8.55399695e-01, -5.17058292e-01, -3.06933810e-02,
            -5.25466391e+03,
           -2.52440457e-01, -3.64417237e-01, -8.96367053e-01,
             6.79074694e+03,
            4.52288820e-01,  7.74500355e-01, -4.42248825e-01,
             8.62335562e+03,
            0.00000000e+00,  0.00000000e+00,  0.00000000e+00,
             1.00000000e+00);

    cout << "Transformed: \n" << r4_c1.inv() * Point4d1 << endl;
